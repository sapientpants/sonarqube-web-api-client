...
describe('CLI Migration Tool', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(MigrationAssistant, 'analyzeUsage').mockReturnValue({
      totalDeprecations: 2,
      byApi: {
        'users.search': 2,
        urgentApi: 1,
    ...
    vi.spyOn(MigrationAssistant, 'generateMigrationGuide').mockReturnValue(
      '# Migration Guide\n\nTest content',
    );
  });
  ...
  describe('MigrationCLI', () => {
    ...
    describe('run method', () => {
      it('should complete successfully when no deprecated APIs found', async () => {
        ...
        vi.spyOn(cli as any, 'scanProject').mockResolvedValue([]);
      ...
      it('should apply migrations in non-dry-run mode', async () => {
        ...
        // Mock applyFix
        vi.spyOn(cli as any, 'applyFix').mockResolvedValue(undefined);
      ...
      it('should handle interactive mode and user cancellation', async () => {
        ...
        vi.spyOn(cli as any, 'prompt').mockResolvedValue('n');
    ...
    describe('applyMigrations method', () => {
      ...
      it('should handle failed automatic fixes', async () => {
        ...
        vi.spyOn(cli as any, 'applyFix').mockRejectedValue(new Error('Fix failed'));
