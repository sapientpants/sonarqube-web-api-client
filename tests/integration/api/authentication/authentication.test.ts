...
(skipTests ? describe.skip : describe)('Authentication API Integration Tests', () => {
  ...
  describe('Token Validation', () => {
    ...
    test('should provide token validation details', async () => {
      ...
      // Cast to any to check for optional properties that may be returned by the API
      const extendedResult = result as { valid: boolean; name?: string; login?: string };
      if (extendedResult.name) {
        expect(typeof extendedResult.name).toBe('string');
      }

      if (extendedResult.login) {
        expect(typeof extendedResult.login).toBe('string');
      }
    });
    ...
    test('should maintain consistent validation results', async () => {
      ...
      const firstResult = validations[0] as { valid: boolean; name?: string; login?: string };
      validations.slice(1).forEach((result) => {
        const extendedResult = result as { valid: boolean; name?: string; login?: string };
        expect(extendedResult.valid).toBe(firstResult.valid);
        if (firstResult.name) {
          expect(extendedResult.name).toBe(firstResult.name);
        }
        if (firstResult.login) {
          expect(extendedResult.login).toBe(firstResult.login);
        }
      });
    });
  });
  ...
  describe('Platform-Specific Authentication', () => {
    test('should handle SonarCloud authentication with organization context', async () => {
      ...
      const { result: projectsResult } = await measureTime(async () => projectsBuilder.execute());
  ...
  describe('Authentication Performance', () => {
    ...
    test('should handle concurrent authentication validations', async () => {
      ...
      const firstResult = results[0] as { valid: boolean; name?: string };
      results.slice(1).forEach((result) => {
        const extendedResult = result as { valid: boolean; name?: string };
        expect(extendedResult.valid).toBe(firstResult.valid);
        if (firstResult.name) {
          expect(extendedResult.name).toBe(firstResult.name);
        }
  ...
  describe('Authentication Response Structure', () => {
    test('should return properly structured authentication response', async () => {
      ...
      const extendedResult = result as { valid: boolean; name?: string; login?: string };
      if ('name' in extendedResult && extendedResult.name) {
        expect(typeof extendedResult.name).toBe('string');
      }

      if ('login' in extendedResult && extendedResult.login) {
        expect(typeof extendedResult.login).toBe('string');
      }
